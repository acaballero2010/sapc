...
export const SapcLogo: React.FC<SapcLogoProps> = ({ 
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <div 
        className="relative flex items-center justify-center shrink-0 rounded-xl overflow-hidden shadow-md bg-white border border-amber-400/50"
        style={{ 
          width: size, 
          height: size,
          background: customLogo ? "#ffffff" : "linear-gradient(135deg, #F5B800 0%, #E5A800 100%)"
        }}
      >
        {customLogo ? (
          <img 
